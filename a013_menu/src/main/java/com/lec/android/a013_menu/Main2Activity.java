...
public class Main2Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        registerForContextMenu(tvCtxMenu);
        registerForContextMenu(ivCtxMenu);
        //unregisterForContextMenu(tvCtxMenu);//등록된메뉴 제거

    }
    // onCreateContextMenu()
    // 컨텍스트 메뉴가 생성될떄 호출되는 콜백 메서드
    @Override
    public void onCreateContextMenu(ContextMenu menu, View v, ContextMenu.ContextMenuInfo menuInfo) {
        super.onCreateContextMenu(menu, v, menuInfo);
        Log.d("dddd","onCreateContextMenu");
      //각각의 뷰 마다 다르게 메뉴 작동하도록 설정 가능
        switch (v.getId()){
            case R.id.tvCtxMenu:
                menu.setHeaderTitle("색깔을 선택하세요");
    ...
    public boolean onContextItemSelected(@NonNull MenuItem item) {
        Log.d("dddd","onContextItemSelected");
        showInfo(item);
        int id=item.getItemId(); //메뉴아이템의 아이디
        int groupId=item.getGroupId(); //메뉴아이템의 그룹아이디
        switch (groupId){
            case 0:
                switch (id){
                    case 1:
                        ...
                        return true;
                    case 2:
                        tvCtxMenu.setTextColor(Color.YELLOW);
                        return true;
                    case 3:
